<?php 
    ...
    if(!isset($_GET['classroom_id']) || $_GET['classroom_id'] == NULL){
        echo "<script> window.location = 'class_list.php' </script>";
    }else {
        $classroom_id = $_GET['classroom_id']; // Lấy brandid trên host
    }
    // gọi class brand
    if($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['submit'])){
        ...
        foreach($get_user as $value){
            if($userid == $value['user_id'] && $username = $value['name']){
                 $update = $conn->query("UPDATE classroom SET name = '$name', description = '$description', staring_date = '$timeline', user_id = $id, user_name = '$username' WHERE classroom_id = '$classroom_id'");
            }
        }
        if(isset($update)){
...
<body>
    <div class="row mx-0">
        ...
        <div class="col-10 px-0 menu_right">
            ...
            <div class="boxInsert">
                <?php 
                    $get_details = $conn->query("SELECT * FROM classroom WHERE classroom_id = '".$classroom_id."'");
                    foreach($get_details as $value){
                ?>
                <form method="post" action="class_edit.php?classroom_id=<?= $classroom_id ?>">
                    <table class="form">                    
                         <?php
                            if(isset($msg_error)){
                        ...
                        <tr>
                            ...
                            <td>
                                <input type="text" value="<?php echo $value['name']; ?>" name="name" />
                            </td>
                        </tr>
                        <tr>
                            <td><label>Description:</label></td>
                            <td>
                                <input type="text" value="<?php echo $value['description']; ?>" name="description" />
                            </td>
                        </tr>
                        <tr>
                            <td><label>Timeline:</label></td>
                            <td>
                                <input type="date" value="<?php echo $value['staring_date']; ?>" name="timeline" />
                            </td>
                        </tr>
                        <tr>
                            <td><label>Lecture ID:</label></td>
                            <td>
                                <input type="text" value="<?php echo $value['user_id']; ?>" name="userid" />
                            </td>
                        </tr>
                        <tr>
                            <td><label>Lecture:</label></td>
                            <td>
                                <input type="text" value="<?php echo $value['user_name']; ?>" name="username" />
                            </td>
                        </tr>
                        <tr> 
                            <td id="td_end"></td>
